...
public class AdministradorDaoImpl implements IAdministradorDao {
	...
	public String getTokenInicioSesion(String atributos, String userdb) {
		...
		String token = null;
		try {
			token = jdbcTemplate.queryForObject("SELECT token.getTokenInicioSesion(?)", new Object[] { atributos },
					String.class);
		} catch (Exception e) {
	...
	public String urltokenPeticion(String userdb) {
		...
		try {
			url = jdbcTemplate.queryForObject(
					"SELECT  wep_valor FROM dbo.web_parametro where wep_nombre like '%TOKEN_URL_AUTENTICACION%' ",
					String.class);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
